...
4. **Ground and answer** (`_shared/prompt.ts` + `_shared/llm.ts`). The full handbook is injected; Workers AI returns schema-guided JSON (answer, citations, confidence, escalation, suggested actions). History roles are clamped to user/assistant so the endpoint cannot be used to inject a system prompt.
...
| `_shared/llm.ts` | Workers AI client + tolerant JSON extraction. The only file to touch to swap LLM provider. |
